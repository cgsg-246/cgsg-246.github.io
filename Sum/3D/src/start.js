...
export function Start() {
    ...
    window.addEventListener('mousemove', (e) => {
        if (!MyLib.isDragging) return;
        const dx = e.clientX - MyLib.lastMouseX;
        const dy = e.clientY - MyLib.lastMouseY;
        MyLib.cameraAngleY -= dx * 0.008;
        MyLib.cameraAngleX += dy * 0.008;
        MyLib.lastMouseX = e.clientX;
        MyLib.lastMouseY = e.clientY;
        drawScene(paramspane.color);
    });
}
